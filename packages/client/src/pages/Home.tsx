...
function Home({ setPage, setUsername }: { setPage: (val: "home" | "game") => void; setUsername: (val: string) => void }) {
	...
	return (
		<div className="tw:min-h-screen tw:p-4 tw:flex tw:items-center tw:flex-col">
			<button className="tw:mb-24 tw:self-start tw:flex tw:items-center tw:bg-amber-900 tw:rounded-full tw:shadow-lg tw:px-4 tw:py-2 tw:text-sm tw:font-medium tw:hover:bg-red-900">
			...
			<div className="tw:flex-grow tw:flex tw:items-center tw:flex-col">
				...
				<input className="tw:shadow-lg tw:mt-6 tw:bg-amber-900 tw:px-5 tw:py-3 tw:rounded-full tw:focus:bg-red-900 tw:focus:outline-none" placeholder="enter a nickname" onChange={(e) => setUsername(e.target.value)} />
				<button className="tw:text-lg tw:font-bold tw:shadow-lg tw:mt-4 tw:bg-amber-700 tw:px-8 tw:py-2 tw:rounded-full tw:hover:bg-red-900" onClick={() => setPage("game")}>
					Play
				</button>
			</div>
